...
def generate_data(size, flog, use_dis=FLAGS.use_dis):
    ...
    print("total" + str(tmp_session_no + size))
    while session_no < tmp_session_no + size:
        print("start session no:"+str(session_no))
        ...
        print("end session no:" + str(session_no))
    next_session = True
...
print("Getting training data: number is %d, average length is %.4f" % (len(data_train), np.mean([len(s) for s in data_train])))
...
def env_train(size, pg=False):
    print("In env_train size = "+ str(size))
    global best_env_train_acc, best_env_train_acc_1
    pre_losses = [1e18] * 3
    for _ in range(size):
        print("Strat epoch " +str(_))
        with env_graph.as_default():
...
def agn_train(size):
    ...
    for _ in range(size):
        ...
        print("------agn train finish epoch-------")
    print("------agn train finish all-------")

def dis_train(size):
    print("In dis_train for size" + str(size))
    ...
    for _ in range(size):
        ...
        print("------dis train finish for epoch-------: " + str(_))
    print("------dis train finish all -------: " + str(_))
...
if FLAGS.interact:
    #env_train(50) #original
    env_train(1)
    interact(1000, use_dis=False)
    if FLAGS.use_dis:
        #dis_train(3) #original
        dis_train(1)
